...
class GPD3303S:
    ...
    def v2(self, value):
        assert (
            value < self.c_limit
        ), f"Current exceeds limit, limit: {self.c_limit}, current: {value}"
    ...
    def output(self, out):
        assert (
            type(out) is bool
        ), f"output expected Boolean, got: {out}, of type {type(out)}"
    ...
    def set_magnet_sweep(self, p):
        ...
        assert (
            f_end < self.c_limit
        ), f"End current exceeds limit, limit: {self.c_limit}, current: {f_end}"
        self.output = True
