...
from pyscan_non_soc_version.general.item_attribute import ItemAttribute
import numpy as np
import struct
...
class Oscilloscope(InstrumentDriver):
    ...
    tdelay_dict = {"ns": 1e-9, "us": 1e-6, "ms": 1e-3}
    plot_size = (8, 6)
    ...
    def read_region(self, edges, channel=0, trig_del="Read", **kwargs):
        ...
        region_edgen = [np.abs(data[0][0] - edge).argmin() for edge in edges]
    ...
    def read_scope_lsection(self, time_window, channel=0, trig_del="Read", **kwargs):
        ...
        section_edge = [
            trdl - tdiv * self.screen_width / 2 + n for n in [0, time_window]
        ]
    ...
    def scope_plot(self, read_func, channel=0, **kwargs):
        ...
        l_str = "Channel {}"
        lines = [
            ax.plot(dat[0], dat[1], plotstyle, label=l_str.format(n))[0]
    ...
    def show_scope_lsection(self, time_window, channel=0, **kwargs):
        self.scope_plot(
            self.read_scope_lsection, channel, time_window=time_window, **kwargs
        )
    ...
    def fourier_signal(self, d, fstart=3, fstop=100):
        ...
        for n in range(3):
            try:
                d.ffit[n] = lor_fit(
                    np.array([d.ffreqs, -d.fourier[n]])[:, fstart:fstop]
                )[0]
            except:
                0
    ...
    def read_vxys(self, d=0, sltime=0, reps=1, init=False, fstart=3, fstop=100):
        ...
        [[d.time, d.volt1], [_, d.volt2], [_, d.volt3], [_, d.volt4]] = (
            self.read_screen(5, init=init)
        )
        if reps > 1:
            for n in range(reps - 1):
                sleep(sltime)
                [[_, v1], [_, v2], [_, v3], [_, v4]] = self.read_screen(5, init=False)
                d.volt1 = v1 + d.volt1
                d.volt2 = v2 + d.volt2
                d.volt3 = v3 + d.volt3
                d.volt4 = v4 + d.volt4
    ...
    def auto_scale(self, channel=1, offset=True, wait=def_wait):
        ...
        volt_limits = [(vblocks - n) * self[vdiv] for n in [-0.2, 0.5]]
        setting_scope = True
        ...
        while setting_scope:
            ...
            elif (vmax < volt_limits[1]) and (offset or (vmin > -volt_limits[1])):
    ...
    def setup_spin_echo(
        ...
        **kwargs
        ...
        """
        delay = 0  # p['pulse1']+p['pulse2']+2*p['delay']
        pre_off = p["h_offset"]
        time_div = p["tdiv"]
        scale = p["scale"]
        self.average = 1
    ...
    def setup_freq_sweep_nolockin(
        self, length, ave=1, scale=0.1, offset=False, **kwargs
    ):
    ...
    def measure_freq_sweep_nolockin(self, params, channel=1, init=True):
        ...
        data = self.read_scope_lsection(
            time_window=length / 1000, channel=channel, init=init
        )[0][1]
        ...
        freqs = np.concatenate(
            [np.ones(sub_length) * fstart + n * fstep for n in nrange]
        )
