...
prop_err_str2 = (
    "set {} property {} to {} but _{} returned {}."
    + "\n This is often a sign of interdependent properties that are not suitable for this auto testing."
    + "Check for interdependence and consider blacklisting."
)
...
def validate_blacklist(test_instrument):
    ...
    if hasattr(test_instrument, "black_list_for_testing"):
        ...
        err_msg = str(
            "Warning, driver needs black_list_for_testing attribute, please double check if there are \n"
...
def restore_initial_state(device, saved_settings):
    ...
    for setting in saved_settings:
        ...
        if _name in device.black_list_for_testing:
            err_msg = (
                "WARNING! BLACKLISTED PROPERTY WAS SOMEHOW CHANGED. Was {}, now {}\n".format(
                    val, device[setter]
                )
        ...
        try:
            ...
            try:
                device[setter] = device["_{}_settings".format(setter)]["return_type"](
                    val
                )
            except Exception:
                assert False, "setter is: {} saved setting is: {} val is: {}".format(
                    setter, setting, val
                )
...
def reset_device_properties(device):
    ...
    for name in settings:
        ...
        if ("values" in keys) and ("indexed_" not in keys) and ("dict_" not in keys):
            ...
            assert (
                False
            ), "no valid type present in setting: {}. Must be one of {}.".format(
                name, ["values", "range", "ranges", "indexed_values", "dict_values"]
            )

    if len(blacklisted) > 0:
        print(
            "These blacklisted settings and their corresponding values were not reset: ",
            blacklisted,
        )
...
def check_has_attributes(device, attributes):
    for a in attributes:
        assert hasattr(
            device, a
        ), "test device does not have {} attribute when initialized".format(a)


# check that the initialized attributes have the expected values
def check_attribute_values(device, attributes, ev):
    for i in range(len(ev)):
        err_string = "test device {} attribute does not equal {}".format(
            device[attributes[i]], ev[i]
        )
...
def check_read_only_property(device, key):
    ...
    # the property value.
    assert (
        type(device[name]) is return_type
    ), "read_only property {} returned type {} not {}".format(
        name, type(device[name]), return_type
    )
    assert (
        type(device["_{}".format(name)]) is return_type
    ), "read_only _property {} returned type {} not {}".format(
        name, type(device["_{}".format(name)]), return_type
    )

    assert (
        "write_string" not in settings
    ), "read_only property {} has write_string {}".format(
...
def check_values_property(device, key):
    ...
    for val in device[key]["values"]:
        ...
        assert device[name] == val, prop_err_str1.format(
            "values", name, val, device[name]
        )
        assert device["_{}".format(name)] == val, prop_err_str2.format(
            "values", name, val, "_{}".format(name), device["_{}".format(name)]
        )
...
def check_range_property(device, key):
    ...
    with pytest.raises(Exception):
        device[name] = min_range - 0.001
    with pytest.raises(Exception):
        device[name] = min_range - 1
    with pytest.raises(Exception):
        device[name] = max_range + 0.001
    with pytest.raises(Exception):
    ...
    for r in range(int(device[key]["range"][0]), int(device[key]["range"][0]), step):
        ...
        assert device["_{}".format(name)] == r, prop_err_str2.format(
            "range", name, r, "_{}".format(name), device["_{}".format(name)]
        )
...
def check_indexed_property(device, key):
    ...
    for idx, iv in enumerate(device[key]["indexed_values"]):
        ...
        device[name] = iv
        assert device[name] == iv, prop_err_str1.format(
            "indexed", name, iv, device[name]
        )
        assert device["_{}".format(name)] == iv, prop_err_str2.format(
            "indexed", name, iv, "_{}".format(name), device["_{}".format(name)]
        )
...
def check_dict_property(device, key):
    ...
    for k in device[key]["dict_values"]:
        ...
        device[name] = k
        err_str1 = (
            "{} key return not properly formatted. Did not return first key {}, instead returned {}"
        ).format(name, device.find_first_key(ord_dict, ord_dict[k]), device[name])
        assert device[name] == device.find_first_key(ord_dict, ord_dict[k]), err_str1
        err_str2 = (
            "_{} key return not properly formatted. Did not return first key {}, instead returned {}"
        ).format(name, device.find_first_key(ord_dict, ord_dict[k]), device[name])
        assert device["_{}".format(name)] == device.find_first_key(
            ord_dict, ord_dict[k]
        ), err_str2
...
def check_properties(test_instrument):
    ...
    print(
        "{} range {} {} total settings found.".format(
            range_counter, mid_string, total_settings
        )
    )
    print(
        "{} values {} {} total settings found.".format(
            values_counter, mid_string, total_settings
        )
    )
    print(
        "{} indexed values {} {} total settings found.".format(
            idx_vals_counter, mid_string, total_settings
        )
    )
    print(
        "{} dict values {} {} total settings found.".format(
            dict_vals_counter, mid_string, total_settings
        )
    )
    try:
        print(
            "{} blacklisted settings not testing (likely due to interdependencies not suitable for automated testing)".format(
                len(test_instrument.black_list_for_testing)
            )
        )
    except Exception:
        pass
    if len(write_only_settings) > 0:
        print(
            "{} write only settings could not be auto tested... custom test cases recommended for this.".format(
    ...
    print(
        "{} properties tested out of {} total settings.".format(
            total_tested, total_settings
        )
    )
    ...
    assert hasattr(
        test_instrument, "_version"
    ), "The instrument had no attribute _version"
...
def write_log(device, exception):
    try:
        ...
        if exception is None:
            ...
            err_string = (
                err_string
                + "failed to log test history. Driver class file path not as expected."
            )
        else:
    ...
    if exception is None:
        ...
        new_line = (
            "Passed with {} version v{} tested on pyscan version {} at {}".format(
                driver_file_name,
                device._version,
                get_pyscan_version(),
        ...
        new_line = (
            "Failed with {} version v{} tested on pyscan version {} at {}".format(
                driver_file_name,
                device._version,
                get_pyscan_version(),
                datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
...
def check_attribute_doc_strings(test_instrument):
    ...
    for setting in settings:
        ...
        assert (
            name in splits[0]
        ), "attribute name not found on first line of doc_string for {}".format(name)
        assert len(splits) > 1, "doc string for {} found as only 1 line".format(name)
        assert [
            len(splits[1]) > 3
        ], "doc string's second line is not long enough for {}".format(name)
...
def check_doc_strings(test_instrument):
    ...
    for line in following_lines:
        ...
        ), "Improper indentation of line {}".format(repr(line))
...
def test_driver(
    ...
    if exception is not None:
        ...
        print(
            "\033[1;32mTests passed, instrument {} looks ready to go.\033[0m".format(
    ...
    print(
        "Tests passed, instrument {} should be ready to go.".format(
            device.__class__.__name__
        )
    )
