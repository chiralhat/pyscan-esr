...
class InstrumentDriver(ItemAttribute):
    ...
    def add_device_property(self, settings):
        ...
        if "values" in settings:
            ...
            assert (
                False
            ), "ranges no longer accepted, must use method to set multiple properties at the same time."
            ...
            assert (
                False
            ), "Key 'values', 'range', indexed_values', 'read_only', or 'dict_values' must be in settings."

        try:
            doc_string = self.get_property_docstring(settings["name"])
        except:
            doc_string = (
                "No doc string found for {}.\n".format(settings["name"])
    ...
    def get_instrument_property(self, obj, settings, debug=False):
        ...
        if not obj.debug:
            ...
            assert isinstance(
                value, str
            ), ".query method for instrument {} did not return string".format(obj)
            value = value.strip("\n")

            if (
                ("values" in settings)
                and ("indexed_" not in settings)
                and ("dict_" not in settings)
            ):
    ...
    def set_values_property(self, obj, new_value, settings):
        ...
        if values.count(new_value) > 0:
            ...
            assert (
                ...
                settings["name"], possible, new_value
            )
    ...
    def set_range_property(self, obj, new_value, settings):
        ...
        if rng[0] <= new_value <= rng[1]:
            ...
            assert (
                False
            ), "Range error: {} must be between {} and {}, cannot be {}".format(
    ...
    def set_indexed_values_property(self, obj, new_value, settings):
        ...
        if new_value in values:
            ...
            if not self.debug:
                ...
                setattr(
                    obj, "_" + settings["name"], settings["write_string"].format(index)
                )
        else:
            possible = []
    ...
    def get_pyscan_properties(self):
        ...
        pyscan_properties = [
            prop.replace("_settings", "") for prop in pyscan_properties
        ]
    ...
    def get_property_docstring(self, prop_name):
        ...
        assert len(match) == 1, "Too many matches for {} documentation".format(
            prop_name
        )
        match = match[0]
