...
def r2bar(data, fit, numfitpars):
    ...
    R2 = 1 - np.sum([(data[i] - fit[i]) ** 2 for i in np.arange(len(data))]) / np.sum(
        [(data[i] - np.mean(data)) ** 2 for i in np.arange(len(data))]
    )
...
def guess_lor_pars(data, skew, full):
    ...
    if skew == 1:
        slope = (
            -(data[1][fitp[0]] - data[1][fitp[1]])
            * minPos
            / datLength
            / (data[0][fitp[1]] - data[0][fitp[0]])
        )
    else:
        slope = 0
...
def lor_fit(data, skew=1, full=1):
    ...
    [GuessB, GuessA, GuessW, GuessF, slope, fitp] = guess_lor_pars(data, skew, full)
...
def plot_lor_fit(data, skew=1, full=1, plt=plt):
    ...
    plt.plot(
        ...
        data[1],
        data[0],
        lorentzianslope(data[0], fit[0], fit[1], fit[2], fit[3], slope, fit[3]),
    )
...
def exp_fit(data, freq, start, end, up, coefs=0):
    ...
    if coefs == 0:
        ...
        guess.append(
            (
                data[1, points[0] : points[1]].max()
                - data[1, points[0] : points[1]].min()
            )
            * up
        )
        ...
        guess = coefs
    fit, err, R2b = func_fit(
        exponential,
        [data[0, points[0] : points[1]], data[1, points[0] : points[1]]],
        guess,
    )
    Q = np.pi * freq * fit[2]
...
def exp_fit_norange(data, freq, up, coefs=0):
    ...
    if coefs == 0:
        ...
        guess.append((data[1].max() - data[1].min()) * up)
...
def plot_exp_fit(
    data, freq, start, end, up=1, coefs=0, plt=plt, col=0, datlab="Data", fitlab="Fit"
):
    ...
    if col == 0:
        plt.plot(
            data[0, points[0] : points[1]],
            data[1, points[0] : points[1]],
            "-",
            label=datlab,
        )
        plt.plot(
            data[0, points[0] : points[1]],
            exponential(data[0, points[0] : points[1]], fit[0], fit[1], fit[2]),
            "--",
            label=fitlab,
        )
    else:
...
def plot_exp_fit_norange(
    data, freq, up=1, coefs=0, plt=plt, col=0, datlab="Data", fitlab="Fit", plotdat=1
):
    """Fits an exponential decay function to a dataset and plot the result.
    ...
    if col == 0:
        ...
        plt.plot(
            data[0], exponential(data[0], fit[0], fit[1], fit[2]), "--", label=fitlab
        )
    else:
        plt.plot(data[0], data[1], ".-", c=col[0], label=datlab)
        plt.plot(
            data[0],
            exponential(data[0], fit[0], fit[1], fit[2]),
            "--",
            c=col[1],
            label=fitlab,
        )
