# -*- coding: utf-8 -*-
from pyscan_non_soc_version.general import set_difference, first_string
import numpy as np
...
class PlotGenerator(object):
    ...
    def get_x(self):
        ...
        self.other_x = set_difference(
            list(self.expt.runinfo.loop0.scan_dict.keys()), self.x_name
        )
    ...
    def get_y(self):
        ...
        if self.expt.runinfo.ndim == 1 and self.y_name is None:
            ...
            if self.y_name is None:
                self.y_name = first_string(
                    list(self.expt.runinfo.loop1.scan_dict.keys())
                )
            self.other_y = set_difference(
                list(self.expt.runinfo.loop1.scan_dict.keys()), self.y_name
            )
    ...
    def get_data(self):
        ...
        if (self.d == 2) and (self.data.ndim > 2):
            if self.index3D is None:
                self.index3D = 0
            self.data_name = self.data_name + "[{}/{}]".format(
                self.index3D, self.data.shape[2]
            )
