...
from pyscan_non_soc_version.measurement.abstract_experiment import AbstractExperiment
from pyscan_non_soc_version.general.is_list_type import is_list_type
import numpy as np
...
class Experiment(AbstractExperiment):
    ...
    def generic_experiment(self):
        ...
        if self.runinfo.time:
            try:
                self.runinfo.dt0 = [0] + [
                    self.runinfo.t0[i] - self.runinfo.t0[i - 1]
    ...
    def average_experiment(self):
        for scan in self.runinfo.scans:
            if isinstance(scan, ps.AverageScan) and (scan.n == 1):
                print(
                    "n_average for average scan is 1. Running generic experiment instead of average experiment."
                )
        ...
        for key, value in data.items():
            ...
            if is_list_type(self[key]):
                ...
                if self.runinfo.average_index == 0:
                    ...
                    self[key] *= self.runinfo.average_index / (
                        self.runinfo.average_index + 1
                    )
                    self[key] += value / (self.runinfo.average_index + 1)
