...
class FastStageExperiment(AbstractExperiment):
    ...
    def setup_instruments(self):
        ...
        runinfo.vel0, runinfo.acc = devices.stage.get_channel_velocity_parameters(
            1
        )  # in mm/s
                ...
                for j in range(self.runinfo.loop1.n):
                    ...
                    for key, value in data.items():
                        if self.runinfo.ndim == 1:
                            ...
                            self[key][:, self.runinfo.indicies[1::]] = np.reshape(
                                np.array(value), (-1, 1)
                            )
    ...
    def line_counts(self, expt):
        ...
        devices.stage.set_channel_velocity_parameters(
            runinfo.fast_chan, runinfo.vel, runinfo.acc
        )
