...
class AbstractExperiment(ItemAttribute):
    ...
    def setup_data_dir(self, data_dir):
        ...
        self.runinfo.data_path = Path(
            data_dir
        )  # seems redundant to use Path() again here
        ...
        with h5py.File(save_name, "a") as f:
            for name in self.runinfo.measured:
                if is_list_type(data[name]) and ndim > 0:
                    ...
                    f.create_dataset(
                        name, shape=dims, fillvalue=np.nan, dtype="float64"
                    )
    ...
    def check_runinfo(self):
        ...
        assert hasattr(
            self.runinfo, "average_d"
        ), "runinfo did not have average d attribute after checking runinfo"
        if self.runinfo.average_d == -1:
            ...
            assert (
                False
            ), "runinfo average d incorrect while has average scan is: " + str(
                self.runinfo.has_average_scan
            )
        ...
        with h5py.File(save_name, "a") as f:
            for key in self.runinfo.measured:
                if not is_list_type(self[key]):
                    ...
                    f[key][self.runinfo.average_indicies, ...] = self[key][
                        self.runinfo.average_indicies, ...
                    ]
                else:
                    f[key][self.runinfo.indicies, ...] = self[key][
                        self.runinfo.indicies, ...
                    ]
        ...
        with h5py.File(save_name, "a") as f:
            for key in self.runinfo.measured:
                if not is_list_type(self[key]):
                    ...
                    f[key][:, self.runinfo.line_indicies, ...] = self[key][
                        self.runinfo.line_indicies, ...
                    ]
                else:
                    f[key][:, self.runinfo.line_indicies, ...] = self[key][
                        self.runinfo.line_indicies, ...
