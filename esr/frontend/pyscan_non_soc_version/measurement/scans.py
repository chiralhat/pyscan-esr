# -*- coding: utf-8 -*-
import numpy as np
from pyscan_non_soc_version.general.same_length import same_length
    ...
    def check_same_length(self):
        ...
        if len(list(self.scan_dict.keys())) > 0:
            if same_length(list(self.scan_dict.values())):
                self.n = len(
                    list(self.scan_dict.values())[0]
                )  # self.n is the length of the input_dict arrays.
...
class AverageScan(AbstractScan):
    ...
    def __init__(self, n_average, dt=0):
        assert isinstance(
            n_average, int
        ), "n_average input for average scan must be an int"
