...
class BKPrecision2190E(Oscilloscope):
    ...
    def initialize_properties(self):
        ...
        self.add_device_property({
            ...
            'query_string': 'TRDL?',
            'range': [-20, 20],
            'return_type': lambda x: float(x.split()[1][:-2])*self.tdelay_dict[x.strip()[-2:]]})
    ...
    def waveform_parameters(self, signal, n):
        blocks = [signal[st:st+end]
                  for st, end in zip(self.block_start, self.block_length)]
        scaler, offset, time_div = [struct.unpack('f', block)[0]
    ...
    def process_waveform(self, sig):
        ...
        if len(sig)>20600:
            ...
            block = sig[21:-2]
        volts = np.array([by if by <= 127 else -1*(256-by) 
                          for by in block])*2*self.scaler[n]-self.offset[n]
